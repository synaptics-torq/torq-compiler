...
// defined in torq_driver.c
extern const char* FLAG_torq_profile_host;
...
public:
  ExecutionContext(TorqHw* torq, iree_hal_torq_native_executable_t* nativeExecutable, TorqDispatchEventLog* eventLog) 
  ...
  iree_status_t enableProfiling(std::string profilingPath) {
    ...
    profilingFile << "id;time_since_open;time_since_start\n";
  ...
  iree_status_t run() {
    ...
    while(actionIndex < actionsCount) {
      ...
      LOGD << "Starting action " << actionIndex;
      ...
      LOGD << "Completed action " << actionIndex;      
...
private:
  ...
  TorqDispatchEventLog* eventLog_{nullptr};
  ...
  iree_status_t processWaitNSSAction(ns(HostAction_table_t) action, ns(WaitNSSParams_table_t) params) {
    ...
    if (profilingFile.good()) {        
        profilingFile << jobId << ";" << t_since_open << ";" << t_since_start << endl;
    }
  ...
  EventType toEventStartType(ns(HostActionParams_union_type_t) paramsType) {
  ...
  EventType toEventEndType(ns(HostActionParams_union_type_t) paramsType) {
  ...
  iree_status_t processAction(ns(HostAction_table_t) action) {
      ...
      if (eventLog_) {
          eventLog_->addEvent(toEventStartType(paramsType), actionIndex);
      }
      ...
      if (eventLog_) {
          eventLog_->addEvent(toEventEndType(paramsType), actionIndex);
      }
...
iree_status_t iree_hal_torq_native_executable_run(
    ...
    TorqDispatchEventLog *eventLogPtr = nullptr;
    
    if (TorqEventLog::isProfilingEnabled()) {
        eventLogPtr = TorqEventLog::get().startDispatch(executable_name);
    }
    ...
    unique_ptr<TorqHw> torq = newTorqHw(FLAG_torq_hw_type, xram_base, xram_size, get_mem_dump_data_root_dir(executable_name), eventLogPtr);
    ...
    ExecutionContext context(torq.get(), executable, eventLogPtr);
