...
void iree_hal_torq_semaphore_state_initialize(
    iree_hal_torq_semaphore_state_t* out_shared_state) {
  memset(out_shared_state, 0, sizeof(*out_shared_state));
  iree_notification_initialize(&out_shared_state->notification);
}

void iree_hal_torq_semaphore_state_deinitialize(
...
static iree_hal_torq_semaphore_t* iree_hal_torq_semaphore_cast(
...
iree_status_t iree_hal_torq_semaphore_create(
  ...
  *out_semaphore = NULL;
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_semaphore_initialize(&iree_hal_torq_semaphore_vtable,
                                  &semaphore->base);
...
static void iree_hal_torq_semaphore_destroy(
    iree_hal_semaphore_t* base_semaphore) {
  iree_hal_torq_semaphore_t* semaphore =
      iree_hal_torq_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_torq_semaphore_query(
...
static iree_status_t iree_hal_torq_semaphore_signal_unsafe(
    iree_hal_torq_semaphore_t* semaphore, uint64_t new_value) {
...
static iree_status_t iree_hal_torq_semaphore_signal(
  ...
  iree_status_t status =
      iree_hal_torq_semaphore_signal_unsafe(semaphore, new_value);
  if (!iree_status_is_ok(status)) {
    iree_slim_mutex_unlock(&semaphore->mutex);
    return status;
  }
...
static void iree_hal_torq_semaphore_fail(iree_hal_semaphore_t* base_semaphore,
                                         iree_status_t status) {
  iree_hal_torq_semaphore_t* semaphore =
      iree_hal_torq_semaphore_cast(base_semaphore);
...
iree_status_t iree_hal_torq_semaphore_multi_signal(
  ...
  for (iree_host_size_t i = 0; i < semaphore_list.count; ++i) {
    ...
    iree_slim_mutex_lock(&semaphore->mutex);
    status = iree_hal_torq_semaphore_signal_unsafe(
...
static bool iree_hal_torq_semaphore_is_signaled(
    iree_hal_torq_semaphore_notify_state_t* state) {
...
static iree_status_t iree_hal_torq_semaphore_wait(
  ...
  iree_notification_await(
      ...
      (iree_condition_fn_t)iree_hal_torq_semaphore_is_signaled,
...
static bool iree_hal_torq_semaphore_any_signaled(
    const iree_hal_semaphore_list_t* semaphore_list) {
  for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
    iree_hal_torq_semaphore_t* semaphore =
        iree_hal_torq_semaphore_cast(semaphore_list->semaphores[i]);
...
// Used with with iree_condition_fn_t and must match that signature.
static bool iree_hal_torq_semaphore_all_signaled(
    const iree_hal_semaphore_list_t* semaphore_list) {
  for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
    iree_hal_torq_semaphore_t* semaphore =
        iree_hal_torq_semaphore_cast(semaphore_list->semaphores[i]);
...
iree_status_t iree_hal_torq_semaphore_multi_wait(
  ...
  if (semaphore_list.count == 0) {
    ...
    return iree_hal_semaphore_wait(semaphore_list.semaphores[0],
                                   semaphore_list.payload_values[0], timeout, IREE_HAL_WAIT_FLAG_DEFAULT);
  }
  ...
  if (iree_timeout_is_immediate(timeout)) {
    iree_status_t status =
        iree_hal_torq_semaphore_result_from_state(wait_mode, semaphore_list);
    IREE_TRACE_ZONE_END(z0);
    return status;
  }
