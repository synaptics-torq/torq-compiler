...
_NEXT_GROUP_HASH = 'c932b937cdd553cab807bca7caed75fce3e791392411865e1ed7817d337c6728'
...
def _skip_next_group_full_model(request, tflite_layer_model):
    """Skip full model tests on next_group (too large)."""
    if _is_full_model_case(tflite_layer_model):
        try:
            ...
            return
        if hashlib.sha256(chip.get('chip_name', '').encode()).hexdigest() == _NEXT_GROUP_HASH:
            pytest.skip(f"Full YOLOv8s-pose model too large for next_group target")
