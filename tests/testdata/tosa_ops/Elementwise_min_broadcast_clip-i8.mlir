module attributes {tfl.description = "Extracted single layer", tfl.schema_version = 3 : i32} {
  func.func @main(%arg0: tensor<1x4x384x384xi8>) -> tensor<1x4x384x384xi8> attributes {tf.entry_function = {inputs = "bert/encoder/layer_0/attention/self/add", outputs = "bert/encoder/layer_0/attention/self/clip_by_value/Minimum"}} {
    %0 = tosa.const_shape  {values = dense<1> : tensor<4xindex>} : () -> !tosa.shape<4>
    %1 = "tosa.const"() <{values = dense<127> : tensor<i8>}> : () -> tensor<i8>
    ...
    %9 = tosa.minimum %6, %8 : (tensor<1x4x384x384xi32>, tensor<1x1x1x1xi32>) -> tensor<1x4x384x384xi32>
