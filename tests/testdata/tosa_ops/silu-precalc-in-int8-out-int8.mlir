// A quantized SiLU (x * sigmoid(x)) taken from YOLOv8n-MIXED: one i8 activation
// feeds both a sigmoid table and an identity rescale, the two arms are multiplied,
// and the product is rescaled back to i8.
//
// Both ends being i8 is what lets PreCalcSiluI8Pattern replay the multiply and the
// rescales at compile time and store the result in the sigmoid table, leaving a
// single 256-entry lookup. Comparing against the llvm-cpu reference, which keeps the
// arms at runtime, is what checks that the precomputation is exact.
//
// Use --torq-disable-precalc-silu-i8 to keep the multiply and compare the two forms.
module {
  func.func @main(%arg0: tensor<1x8x8x16xi32>) -> tensor<1x8x8x16xi8> {
    %lut = "tosa.const"() <{values = dense<""> : tensor<256xi8>}> : () -> tensor<256xi8>
    %in_mult = "tosa.const"() <{values = dense<1361387520> : tensor<1xi32>}> : () -> tensor<1xi32>
    %in_shift = "tosa.const"() <{values = dense<39> : tensor<1xi8>}> : () -> tensor<1xi8>
    ...
    %act = tosa.rescale %arg0, %in_mult, %in_shift, %zero_i32, %act_zp {input_unsigned = false, output_unsigned = false, per_channel = false, rounding_mode = DOUBLE_ROUND, scale32 = true} : (tensor<1x8x8x16xi32>, tensor<1xi32>, tensor<1xi8>, tensor<1xi32>, tensor<1xi8>) -> tensor<1x8x8x16xi8>
