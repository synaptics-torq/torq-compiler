...
"""Artifact discovery, provenance, and manifest persistence.

``describe`` resolves a VMFB/model's metadata by precedence; ``build_manifest``
and the atomic writers persist the run manifest (``manifest.json``).
"""
...
def build_manifest(
    *,
    ...
    ``artifact`` is an optional ``torq.lab.pipeline.artifacts.ArtifactInfo`` describing
