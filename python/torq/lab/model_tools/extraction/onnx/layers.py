...
"""ONNX layer extraction: layer cases, tensor folding, layer construction, and cache.

Splits an ONNX model into standalone per-layer models (with a disk-backed
cache) and wraps them as test cases. General subgraph matching/extraction
lives in :mod:`torq.lab.model_tools.extraction.onnx.subgraphs`; ONNX-to-MLIR
import in :mod:`torq.lab.model_tools.importers.onnx`. Used by the
``torq.gen_config`` discovery path; ``torq.testing.onnx`` re-exports these
names and adds fixtures on top.
"""
...
from torq.lab.pipeline.artifacts import atomic_write_json_manifest

def _fix_batch_dimension_to_one(model: onnx.ModelProto) -> int:
    """Replace an unspecified first tensor dimension with a static batch of one."""
    modified_count = 0
